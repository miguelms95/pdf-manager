...
public class VentanaPrincipal extends JFrame {
	...
	private JList<String> listaImportados;
...
//			listaImportados = new JList<String>();
	...
	private void imprimirLista() {
		if(!archivos.isEmpty()){
			for (File file : archivos) {
				if(!modeloLista.contains(file.getName()))
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(seleccionados.length>0){
						for (int i=0; i< seleccionados.length;i++) {
							...
							if(!modeloLista.contains(elemento))
...
//	}
		...
		if (btSubir == null) {
			...
			btSubir.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					if(ant >=0){
						String aux = modeloLista.getElementAt(ant);
						modeloLista.setElementAt(modeloLista.getElementAt(index), ant);
						modeloLista.setElementAt(aux, index);
		...
		if (btBajar == null) {
			...
			btBajar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					if(next < modeloLista.getSize()){
						String aux = modeloLista.getElementAt(next);
						modeloLista.setElementAt(modeloLista.getElementAt(index), next);
						modeloLista.setElementAt(aux, index);
			...
			btnProcesar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					case "Extraer paginas":
						...
						VentanaExtraerPaginas ve;
						try {
							ve = new VentanaExtraerPaginas("C:\\Users\\migue\\Desktop\\HERENCIA GENETICA.pdf");
							ve.setVisible(true);
						} catch (IOException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
	...
	private JList<String> getListAdded() {
		if (listaSeleccionados == null) {
			listaSeleccionados = new JList<String>();
			modeloLista = new DefaultListModel<>();
			listaSeleccionados.setModel(modeloLista);
	...
	private void eliminarElementos(){
		int[] valores = listaSeleccionados.getSelectedIndices();
		while(valores.length > 0){
			Object obj = modeloLista.getElementAt(valores[0]);
			int indexElementoBorrar = find(obj);
			modeloLista.remove(indexElementoBorrar);
			archivos.remove(valores[0]);
			
//			modeloLista.removeElement(modeloLista.getElementAt(valores[0]));
			valores = listaSeleccionados.getSelectedIndices();
		}
		habilitarBotones();
	}

	private int find(Object element) {
		return modeloLista.indexOf(element);
	}
	...
	private void seleccionarTodos(JList<String> lista){
	...
	private void unirPDFs() throws IOException{
		...
		if(indicesSeleccionados.length > 1){
			for(int i=0; i <indicesSeleccionados.length;i++){
				listaArchivos.add(archivos.get(findIndexArchivo(modeloLista.get(indicesSeleccionados[i]))));
			}
			...
			String userDir = System.getProperty("user.home");
			String cadena = modeloLista.firstElement();//.get(listaSeleccionados.getSelectedIndex()).getName();
			...
			JOptionPane.showMessageDialog(this, "Tienes que importar más de un pdf para unir.");
	}
	...
	private void habilitarBotones(){
		...
		if(modeloLista.size()>0){
			mntmReiniciar.setEnabled(true);
		}else{
			mntmReiniciar.setEnabled(false);
		}
		
		// siempre actualizo los botones de subir y bajar
		if(modeloLista.size()>= 2){
